...
class WCAF(nn.Module):
    def __init__(
        ...
        self.qkv_bias = True
        # 特征对齐（增加 LayerNorm）
        # self.norm_SRC = LN2d(embed_dims)
        # self.norm_TGT = LN2d(embed_dims)
...
class WCAF_RGB(nn.Module):
    def __init__(
        ...
        self.depth_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, padding=1)
    ...
    def forward(self, SRC, TGT):
        ...
        depth_bias = self.depth_conv(SRC.permute(0, 2, 1).reshape(B, C, H, W))  # (B, C, H, W)
        depth_bias = self.depth_bias_proj(depth_bias.flatten(2).permute(0, 2, 1))  # (B, seq_len, 1)
        # depth_bias = self.depth_bias_proj(depth_bias)  # [B, seq_len, embed_dims]
        ...
        K = self.K_proj_SRC(SRC)  # (B, seq_len, C)
        Q = self.Q_proj_TGT(TGT)  # (B, seq_len, C)
        # Q = self.Q_proj_TGT(TGT + depth_bias)  # (B, seq_len, embed_dims)
        Vs = self.V_proj_SRC(SRC)  # (B, seq_len, C)
        Vt = self.V_proj_TGT(TGT)  # (B, seq_len, C)
        ...
        # _, At = self.attn(Q, K, Vt)  # At: (B, seq_len, seq_len)
        attn = (Q * self.scale) @ K.transpose(-2, -1) # (B, seq_len, seq_len)
        attn += self.gamma_depth * depth_bias
        attn = attn.softmax(dim=-1)
        # **计算加权 V**
        x1 = attn @ Vs  # (B, seq_len, C)
        x2 = attn @ Vt  # (B, seq_len, C)
