...
class ColorAttentionAdapter(nn.Module):
    ...
    def forward(self, x):
        if self.change_channel: # 执行
            x = x.permute(0, 3, 1, 2).contiguous() # (2, 32, 32, 1280) -> (2, 1280, 32, 32)
            avg_out = self.fc2(self.act(self.fc1(self.avg_pool(x)))) # (2, 1280, 1, 1)
            max_out = self.fc2(self.act(self.fc1(self.max_pool(x)))) # (2, 1280, 1, 1)
            return self.Sigmoid(avg_out + max_out).view(x.shape[0], 1, 1, -1) # (2, 1, 1, 1280)
        else: # 跳过
...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        out = self.fc2(self.act(self.fc1(x))) # (18, 14, 14, 1280)
        if self.skip:
            out = out + x
        return self.scale * out # (18, 14, 14, 1280)
