...
from my_utils import Fuse_Block
...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        self.kongjian = Get_Kongjian(256, 256)
        self.tongdao = Get_Tongdao(256, 256)
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        '''
        edge_attn = self.kongjian(depth_outputs[1][0],rgb_outputs[1][0])
        # edge_attn = self.kongjian(depth_outputs[1][1],rgb_outputs[1][1])
        pixel_attn = self.tongdao(depth_outputs[1][1],rgb_outputs[1][1])

        # vision_outputs = self.alpha * (rgb_outputs[1][1] * edge_attn) + (1 - self.alpha) * (rgb_outputs[1][1] * pixel_attn)
        vision_outputs = rgb_outputs[1][1] + rgb_outputs[1][1] * edge_attn * pixel_attn # 下面这行跑完，本行解除注释，调大学习率，再训练一次
        #vision_outputs = rgb_outputs[1][1] + rgb_outputs[1][0] * edge_attn + rgb_outputs[1][1] * pixel_attn
...
#         self.conv_x.weight.data = sobel_x
#         self.conv_y.weight.data = sobel_y
#         self.conv_x.requires_grad_(False)
#         self.conv_y.requires_grad_(False)
#
#         # 权重映射（对应公式中的 W_e 和 b_e）
#         self.weight_net = nn.Sequential(
#             nn.Conv2d(1, 1, kernel_size=1),  # W_e 是1x1卷积核
#             nn.Sigmoid()
#         )
#
#     def forward(self, depth):
#         gx = self.conv_x(depth)
#         gy = self.conv_y(depth)
#         grad_mag = torch.sqrt(gx ** 2 + gy ** 2 + 1e-6)  # M_edge
#         return self.weight_net(grad_mag)  # W_edge
#
# class PixelWeightGenerator(nn.Module):
#     def __init__(self, kernel_size=5):
#         super().__init__()
#         # 均值池化（局部均值计算）
#         self.avg_pool = nn.AvgPool2d(kernel_size, stride=1, padding=kernel_size//2)
#         # 权重映射（对应公式中的 W_p 和 b_p）
#         self.weight_net = nn.Sequential(
...
#
#     def forward(self, depth):
#         mu = self.avg_pool(depth)
#         var = self.avg_pool((depth - mu)**2) # Var_local
#         return self.weight_net(var) # W_pixel
#
#
# class MultiGranularityFusion(nn.Module):
#     def __init__(self, in_channels):
#         super().__init__()
#         self.edge_weight = EdgeWeightGenerator()
#         self.pixel_weight = PixelWeightGenerator()
#         # 边缘增强分支的卷积（对应公式中的 Conv）
#         self.edge_conv = nn.Conv2d(2 * in_channels, in_channels, kernel_size=3, padding=1)
#
#     def forward(self, rgb_feat, depth_feat, depth_map):
#         # 边缘增强分支
#         edge_mask = self.edge_weight(depth_map)
#         edge_feat = self.edge_conv(torch.cat([rgb_feat, depth_feat], dim=1)) * edge_mask
#
#         # 细节增强分支
#         pixel_mask = self.pixel_weight(depth_map)
#         pixel_feat = (rgb_feat + depth_feat) * pixel_mask
#
#         # 最终融合
#         return edge_feat + pixel_feat

class GEA(nn.Module):
    def __init__(self):
        super(GEA, self).__init__()
        self.reduce1 = Conv1x1(768, 256)
        self.reduce4 = Conv1x1(768, 256)
    ...
    def forward(self, x1, x4):
        x1 = self.reduce1(x1) # (b, 256, h, w)=(2,256,64,64)
        x4 = self.reduce4(x4) # (b, 256, h, w)

        gradient_x1 = self.compute_image_gradient(x1) # (b, 256, h, w)
        gradient_x4 = self.compute_image_gradient(x4) # (b, 256, h, w)

        x1_with_gradient = torch.cat((x1, gradient_x1), dim=1) # (b, 512, h, w)
        x4_with_gradient = torch.cat((x4, gradient_x4), dim=1) # (b, 512, h, w)

        attention_weights = self.attention(torch.cat((x4_with_gradient, x1_with_gradient), dim=1)) # (b, 1024, h, w) ->  # (b, 1, h, w)
        x1_weighted = x1_with_gradient * attention_weights[:, :, :x1.size(2), :x1.size(3)] # (b, 512, h, w)
        x4_weighted = x4_with_gradient * attention_weights[:, :, :x1.size(2), :x1.size(3)] # (b, 512, h, w)
        out = torch.cat((x4_weighted, x1_weighted), dim=1) # (b, 1024, h, w)
        out = self.block(out) # (b, 1, h, w)
    ...
    def compute_image_gradient(self, x):
        ...
        # return gradient_x
        # """ 使用 torch.gradient() 计算特征图的梯度 """
        G_y, G_x = torch.gradient(x, dim=[-2, -1])  # 在 h, w 方向计算梯度
...
class GEB(nn.Module):
    def __init__(self):
        ...
        self.avg_pool = nn.AvgPool2d(kernel_size=5, stride=1, padding=5//2)
    ...
    def compute_image_gradient(self, x):
        mu = self.avg_pool(x)
        var = self.avg_pool((x - mu)**2) # Var_local
        return self.weight_net(var) # W_pixel


class Get_Tongdao(nn.Module):
    def __init__(self, rgb_channels, depth_channels):
        super().__init__()
        # 步骤1：将RGB和深度特征拼接后降维
        self.fusion_conv = nn.Conv2d(rgb_channels + depth_channels, rgb_channels, kernel_size=1)
    ...
    def forward(self, depth_feat, rgb_feat):
        ...
        # return weighted_feature
        return attention # [B, rgb_c, 1, 1]

class Get_Kongjian(nn.Module):
    def __init__(self, rgb_channel, depth_channel):
        super().__init__()
        self.reduce_768 = nn.Conv2d(rgb_channel + depth_channel, rgb_channel,1)
        self.reduce_1 = nn.Conv2d(rgb_channel + depth_channel, rgb_channel,1)
        # self.reduce_2 = nn.Conv2d(256, 1, 1)
        ...
        self.block = nn.Sequential(
            nn.Conv2d(rgb_channel, rgb_channel // 16, 1),
            # nn.BatchNorm2d(rgb_channel // 16),
            LN2d(rgb_channel // 16),
            nn.ReLU(inplace=True),
            nn.Conv2d(rgb_channel // 16, rgb_channel, 1),
            nn.Sigmoid(),
        )

    def forward(self, depth, rgb):
        fused = torch.cat((depth, rgb), dim=1) # (b,256*2,32,32)
        fused = self.reduce_768(fused) # (b,256,32,32)
        # x = self.reduce_1(fused) # (b,1,32,32)
        gradient = self.compute_image_gradient(fused) # (b,256,32,32)
        x1 = self.reduce_1(torch.cat((fused, gradient), dim=1)) # (b,256,32,32)
        out = self.block(x1)
    ...
    def compute_image_gradient(self, x):
        ...
        # 由于每个通道都需要应用相同的核，因此需要重复卷积核以适应多通道
        channels = x.shape[1]
        sobel_kernel_x = sobel_kernel_x.repeat(channels, 1, 1, 1)
        sobel_kernel_y = sobel_kernel_y.repeat(channels, 1, 1, 1)
        # 使用 groups 参数对每个通道独立卷积，保持输出通道与输入通道一致
        grad_x = F.conv2d(x, sobel_kernel_x, padding=1, groups=channels)
        grad_y = F.conv2d(x, sobel_kernel_y, padding=1, groups=channels)

        return torch.sqrt(grad_x**2 + grad_y**2 + 1e-6) # (b,c,32,32)