...
class SAM_Anchor_Prompt(MaskRCNN):
    ...
    def predict(self,
        ...
        if batch_data_samples[0].get('proposals', None) is None: # 执行
...
class MyPrompterAnchorRoIPromptHead(StandardRoIHead):
    ...
    def loss(self,
        ...
        if self.with_mask:
            mask_results = self.mask_loss(
                x, # 多尺度特征图
                sampling_results,
                bbox_results['bbox_feats'], # 根据 RoI，从多尺度特征图中提取出的特征矩阵(512, 256, 7, 7)，不过预测 mask时要重新根据 RoI提取特征图(17, 256, 14, 14)，bbox_feats似乎没用了
                batch_gt_instances,
                # query_features=query_features,
                # query_locations=query_locations,
                image_embeddings=image_embeddings,
    ...
    def predict(self,
        ...
        if hasattr(self, 'extra_pe'):
            bs, _, h, w = x[0].shape
            mask_pe = torch.zeros((bs, h, w), device=x[0].device, dtype=torch.bool) # 全是 False
            img_feats_pe = self.extra_pe(mask_pe)
            outputs = []
