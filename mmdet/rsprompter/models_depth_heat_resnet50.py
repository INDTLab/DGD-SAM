...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        res_config = ResNetConfig.from_pretrained("/data2/yihan/MyProject/RSPrompter-release/Local_Models_weights/ResNet-50/")
        self.depth_backbone = ResNetModel.from_pretrained("/data2/yihan/MyProject/RSPrompter-release/Local_Models_weights/ResNet-50/", config=res_config)
        # self.depth_backbone输出的特征图形状为(b, 2048, 512/32, 512/32)
        # self.depth_neck = nn.Sequential(
        ...
        self.edge_attn = EdgeAttention(in_channels=64)
        self.pixel_attn = PixelAttention(in_channels=2048)
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        depth_outputs = self.depth_backbone(depth_inputs, output_hidden_states=True).hidden_states # tuple_5, (b, 64, 128, 128),(b, 256, 128, 128),(b, 512, 64, 64),(b, 1024, 32, 32),(b, 2048, 16, 16)
        # depth_outputs = F.interpolate(depth_outputs, size=(32, 32), mode='bilinear', align_corners=False) # (2, 2024, 32, 32)
        # depth_outputs = self.depth_neck(depth_outputs) # (2, 256, 16, 16)
        edge_mask = self.edge_attn(depth_outputs[0])
        pixel_mask = self.pixel_attn(depth_outputs[4])
...
class EdgeAttention(nn.Module):
    def __init__(self, in_channels=64, out_channels=256):
        ...
        self.edge_conv = nn.Sequential(
            nn.Conv2d(in_channels, 128, kernel_size=4, padding=1, stride=2),
            LayerNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, out_channels, kernel_size=4, padding=1, stride=2),
