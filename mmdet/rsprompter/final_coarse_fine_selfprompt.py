...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        self.hf_attn = HighFreqAttention(kernel_size=3)
        # self.hf_attn = PixelAttention(in_channels=256, out_channels=256)
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # edge_mask = self.edge_attn(depth_outputs[1][1])
        hf_mask = self.hf_attn(depth_outputs[1][1])
        pixel_mask = self.pixel_attn(depth_outputs[1][1])
        vision_outputs = self.alpha * (rgb_outputs[1][1] * hf_mask) + (1 - self.alpha) * (rgb_outputs[1][1] * pixel_mask)
        # vision_outputs = rgb_outputs[1][1] + rgb_outputs[1][1] * edge_mask * pixel_mask

        '''
        import matplotlib.pyplot as plt
        plt.subplot(1, 2, 1)
        plt.imshow(edge_mask[0, 0].detach().cpu().numpy(), cmap='viridis')
        plt.title('Edge Attention Map')

        plt.subplot(1, 2, 2)
        plt.imshow(pixel_mask[0, 0].detach().cpu().numpy(), cmap='viridis')
        plt.title('Pixel Attention Map')
        plt.show()
        '''

        vision_outputs = (vision_outputs,)
        # vision_outputs = (rgb_outputs[1][1],)
        # vision_outputs = (self.CBAM(rgb_outputs[1][1], depth_outputs[1][1]),)
        # vision_outputs = (self.AttnFusion(rgb_outputs[1][1], depth_outputs[1][1]),)
...
class MyPrompterAnchorRoIPromptHead(StandardRoIHead):
    def __init__(
        ...
        self.Fuse = AttentionFusion(32,32, 4)
    ...
    def mask_loss(self, x: Tuple[Tensor],
        ...
        hq_feature = self.Fuse(self.embedding_encoder(image_embeddings_tmp), self.compress_vit_feat(vit_feature)) # (17,32,128,128), 试试这此处用SEBlock融合
...
class AttentionFusion(nn.Module):
    def __init__(self, rgb_channels, depth_channels, div = 16):
        super().__init__()
        # 步骤1：将RGB和深度特征拼接后降维
        ...
        self.se_block = nn.Sequential(
            ...
            nn.Conv2d(rgb_channels, rgb_channels // div, kernel_size=1),  # 降维
            nn.ReLU(),
            nn.Conv2d(rgb_channels // div, rgb_channels, kernel_size=1),  # 升维
            nn.Sigmoid()  # 输出0~1的权重
        )
...
def sobel_filter(depth_map):
    """ 使用 Sobel 过滤器提取高频特征 """
    sobel_x = torch.tensor([[[[-1, 0, 1],
                              [-2, 0, 2],
                              [-1, 0, 1]]]], dtype=torch.float32, device=depth_map.device)
    sobel_y = torch.tensor([[[[-1, -2, -1],
                              [0, 0, 0],
                              [1, 2, 1]]]], dtype=torch.float32, device=depth_map.device)
    sobel_x = sobel_x.expand(depth_map.shape[1], 1, 3, 3)
    sobel_y = sobel_y.expand(depth_map.shape[1], 1, 3, 3)

    edge_x = F.conv2d(depth_map, sobel_x, padding=1, groups=depth_map.shape[1])
    edge_y = F.conv2d(depth_map, sobel_y, padding=1, groups=depth_map.shape[1])

    high_freq = torch.sqrt(edge_x ** 2 + edge_y ** 2)  # 计算梯度幅值
    return high_freq
# 生成高频特征权重
class HighFreqAttention(nn.Module):
