...
def save_first_mask(mask_preds, batch_img_metas, iteration_name):
    """
    保存每张图片的第一个mask

    Args:
        mask_preds: mask预测结果 (N, 1, H, W)
        batch_img_metas: 图像元信息
        iteration_name: 'first' 或 'last'
    """
    import torch.nn.functional as F

    save_dir = Path(f"/data2/yihan/tmp/tmp/mask_visualization_viridis/{iteration_name}_iteration")
    ...
    # 保存第一个mask (保留原始预测数值)
    mask = mask_resized[0, 0].detach().cpu().numpy()

    # 归一化到0-255用于显示 (保留原始数值的相对关系)
    mask_min = mask.min()
    mask_max = mask.max()
    if mask_max - mask_min > 0:
        mask_normalized = ((mask - mask_min) / (mask_max - mask_min) * 255).astype(np.uint8)
    else:
        mask_normalized = (mask * 0).astype(np.uint8)

    # 应用jet颜色映射
    mask_jet = cv2.applyColorMap(mask_normalized, cv2.COLORMAP_VIRIDIS)

    # 获取图像文件名
    img_filename = batch_img_metas[0].get('img_path').split('/')[-1].split('.')[0]

    save_path = save_dir / f"{img_filename}_mask_0.png"
    cv2.imwrite(str(save_path), mask_jet)
    print(f"Saved {iteration_name} mask: {save_path}")
...
class SAM_Anchor_Prompt(MaskRCNN):
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # edge_mask = self.edge_attn(depth_outputs[1][0])
        # pixel_mask = self.pixel_attn(depth_outputs[1][1])
        # vision_outputs = self.alpha * (rgb_outputs[1][1] * edge_mask) + (1 - self.alpha) * (rgb_outputs[1][1] * pixel_mask)
        # vision_outputs = rgb_outputs[1][1] + rgb_outputs[1][1] * edge_mask * pixel_mask

        # vision_outputs = (vision_outputs,)
        vision_outputs = (rgb_outputs[1][1],)
        ...
        vit_features = rgb_outputs[0][1] # (b, 768, 32, 32)，一开始用的layer_11
        # vit_features = rgb_outputs[0][0] # (b, 768, 32, 32)，改成layer_2的试试
    ...
    def predict(self,
        ...
        # os.makedirs("/data2/yihan/tmp/tmp", exist_ok=True)
        # plt.imshow(F.interpolate(x[2][:, :, :, :], size=(380, 640), mode='bilinear', align_corners=False)[0, :, :, :].mean(0).detach().cpu().numpy(), cmap='jet')
        # plt.savefig(f"/data2/yihan/tmp/tmp/{batch_data_samples[0].get('img_path').split('/')[-1].split('.')[0]}.png",bbox_inches='tight', pad_inches=0, dpi=300)
        # plt.close()
...
class MyPrompterAnchorRoIPromptHead(StandardRoIHead):
    ...
    def predict_mask(
        ...
        # 保存第一次的mask
        # save_first_mask(mask_results['mask_preds'], batch_img_metas, 'first')
        ...
        # 保存最后一次的mask
        # save_first_mask(mask_results['mask_preds'], batch_img_metas, 'last')
...
@MODELS.register_module()
class PseudoFeatureAggregatorV2(BaseModule):
    def __init__(
        self,
        in_channels,
        hidden_channels=256,
        ...
        self.norm1 = LayerNorm2d(hidden_channels)
        self.act = nn.GELU()
        self.conv2 = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=3, padding=1, bias=False)
        self.norm2 = LayerNorm2d(hidden_channels)
        self.conv3 = nn.Conv2d(hidden_channels, out_channels, kernel_size=3, padding=1, bias=False)
...
@MODELS.register_module()
class RSSimpleFPNV2(BaseModule):
    def __init__(self,
        ...
        # learnable weights for top-down fusion
        self.alpha = nn.Parameter(torch.ones(self.num_ins - 1))
        if use_attention:
    ...
    def forward(self, inputs):
        ...
        for i in range(len(laterals) - 1, 0, -1):
            fusion = F.interpolate(laterals[i], size=laterals[i - 1].shape[-2:], mode='nearest')
            laterals[i - 1] = laterals[i - 1] + weights[i - 1] * fusion
...
@MODELS.register_module()
class NULLPseudoFeatureAggregator(BaseModule):
    def __init__(
        self,
        in_channels,
        hidden_channels=256,
