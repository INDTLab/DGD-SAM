...
class SAM_Mask_Decoder(RSSamMaskDecoder):
    def __init__(self, *args, **kwargs):
        ...
        sam_config = SamConfig.from_pretrained(kwargs.get('hf_pretrain_name')).mask_decoder_config
        # sam_config.update(dict(num_multimask_outputs=7)) # 设为7，就可以得到 8个self.mask_decoder.num_mask_tokens和 8个相应的mlp层
        # sam_config.update(dict(num_multimask_outputs=3))
        self.mask_decoder = SamMaskDecoder(sam_config)

        self.pre1 = nn.Sequential(nn.ConvTranspose2d(256, 32, 2, 2, 0), conv_up_pre(32, 32))
        self.fu1 = conv_block(256, 256)
        self.de12 = conv_up0(768, 256)

        # self.mask_decoder.mask_tokens = nn.Embedding(8, 256)
        # self.mask_decoder.num_mask_tokens = 8
    ...
    def conn_forward(
        ...
        # 从这里开始
        ori_mask1_ = self.fu1(image_embeddings) # b,256,32,32
        ori_mask_pre1 = self.pre1(ori_mask1_) # b,8,64,64

        return ori_mask_pre1
...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        print(1)
        ''' yihan '''
        # 保存模型的参数，运行一次后就注释掉，用于修改predict函数时，给test.py提供一个.pth
...
class MyPrompterAnchorRoIPromptHead(StandardRoIHead):
    ...
    def _mask_forward_2(self,
        ...
        mask_as_prompt_preds, iou_predictions, conn_mask = self.mask_head.mask_as_prompt_forward(
            mask_feats, # (17, 256, 14, 14)
        ...
        mask_results = dict(mask_preds=mask_as_prompt_preds, mask_feats=mask_feats, iou_predictions=iou_predictions, conn_mask=conn_mask)
        return mask_results
    ...
    def mask_loss(self, x: Tuple[Tensor],
        ...
        # mask_prompt = bv_test(mask_results['mask_preds'])
        for i in range(2, self.decoder_layers + 1):
            mask_results_2 = self._mask_forward_2(
                x=x,  # tuple_5
                rois=pos_rois, # (17, 5)
                mask_as_prompt=mask_results['mask_preds'], # (17, 1, 128, 128)
                # mask_as_prompt=bv_test(mask_results['mask_preds']), # (17, 1, 128, 128)
                image_embeddings=image_embeddings, # (2, 256, 32, 32)
                image_positional_embeddings=image_positional_embeddings, # (2, 256, 32, 32)
            )
            mask_loss_and_target_2, loss_conn_mask = self.mask_head.loss_and_target_conn(
                mask_preds=mask_results_2['mask_preds'], # (17, 1, 128, 128)
                conn_mask=mask_results_2['conn_mask'],
                sampling_results=sampling_results,
                batch_gt_instances=batch_gt_instances,
                rcnn_train_cfg=self.train_cfg)
            ...
            if hasattr(self.mask_head, 'loss_boundary'):
                mask_results.update({
                    ...
                    f'loss_conn_mask': loss_conn_mask,
                })
            else:
                mask_results.update({
                    f'loss_mask{i}': mask_loss_and_target_2['loss_mask']['loss_mask'],
    ...
    def loss(self,
        ...
        if hasattr(self, 'extra_pe'):
            ...
            img_feats_pe = self.extra_pe(mask_pe) # (2, 256, 128~8, 128~8)
            outputs = []
        ...
        if self.with_mask:
            ...
            for i in range(2, self.decoder_layers + 1):
                ...
                losses.update({f'loss_conn_mask': mask_results['loss_conn_mask']})
    ...
    def predict_mask(
        ...
        # mask_preds = bv_test(mask_results['conn_mask'])
...
class MyMaskHead(RSPrompterAnchorMaskHead):
    ...
    def mask_as_prompt_forward(self,
        ...
        conn_mask = self.mask_decoder.conn_forward(
        # low_res_masks, iou_predictions, mask_decoder_attentions = self.mask_decoder2(
            image_embeddings=image_embeddings,  # (batch=17, C=256, H=32, W=32)
            image_positional_embeddings=image_positional_embeddings,  # (batch=17, C=256, H=32, W=32)
            sparse_prompt_embeddings=sparse_embeddings,  # (batch=17, 1, 5, 256)
        ...
        return low_res_masks, iou_predictions, conn_mask
    ...
    def loss_and_target(self, mask_preds: Tensor,
        ...
        # 对每个掩码应用 sal2conn
        '''
        conn_targets = torch.zeros(mask_targets.shape[0], 8, mask_targets.shape[1], mask_targets.shape[2]).to(mask_preds.device)
        for i in range(mask_targets.shape[0]):
            conn_targets[i] = sal2conn(mask_targets[i])
        '''


        # mask_targets = conn_targets # (17, 8, 512, 512)
        ...
        if mask_preds.size(0) == 0:
            ...
            if self.class_agnostic: # 执行这里，因为 mask decoder预测的是二值掩码，而类别在生成 RoI时就已经分配好了
                ...
                loss_mask = self.loss_mask(mask_preds, mask_targets,
                                           pos_labels)
        # loss_mask = torch.nn.functional.binary_cross_entropy_with_logits(mask_preds,conn_targets)
    ...
    def loss_and_target_conn(self, mask_preds: Tensor,
                        conn_mask: Tensor,
                        sampling_results: List[SamplingResult],
                        batch_gt_instances: InstanceList,
                        rcnn_train_cfg: ConfigDict) -> dict:
        ...
        # for i in range(mask_targets.shape[0]):
        #     conn_targets[i] = sal2conn(mask_targets[i])
        conn_targets = sal2conn_batch(mask_targets)

        conn_mask = F.interpolate(conn_mask, size=mask_targets.shape[-2:], mode='bilinear', align_corners=False)
        loss_conn_mask = torch.nn.functional.binary_cross_entropy_with_logits(conn_mask,conn_targets)

        return dict(loss_mask=loss, mask_targets=mask_targets), loss_conn_mask
...
Directable={'upper':[0,-2],'up':[0,-1],'lefter':[-2,0],'left':[-1,0],'right':[1,0],'righter':[2,0],'down':[0,1],'downer':[0,2]}
TL_table = ['downer','down','righter','right','left','lefter','up','upper']
def sal2conn(mask):
    ## converte the saliency mask into a connectivity mask
    ## mask shape: H*W, output connectivity shape: 8*H*W
    [rows, cols] = mask.shape # [768, 1024]
    conn = torch.zeros([8,rows, cols]).to(mask) # (8, 768, 1024)
    # 下面这些变量表示偏移后的 mask
    up = torch.zeros([rows, cols]).to(mask) # move the orignal mask to up, 代表初始 mask向 上 移动 1 个像素后的结果
    down = torch.zeros([rows, cols]).to(mask)
    left = torch.zeros([rows, cols]).to(mask) # 初始 mask向 左 移动 1 个像素后的结果
    right = torch.zeros([rows, cols]).to(mask)
    upper = torch.zeros([rows, cols]).to(mask) # 初始 mask向 上 移动 2 个像素后的结果
    righter = torch.zeros([rows, cols]).to(mask)
    lefter = torch.zeros([rows, cols]).to(mask) # 初始 mask向 左 移动 2 个像素后的结果
    downer = torch.zeros([rows, cols]).to(mask)

    # 计算偏移后的掩码
    up[:rows-1, :] = mask[1:rows,:] # mask 向上移动 1 像素
    down[1:rows,:] = mask[0:rows-1,:]
    left[:,:cols-1] = mask[:,1:cols]
    right[:,1:cols] = mask[:,:cols-1]
    upper[:rows-2, :] = mask[2:rows,:]
    downer[2:rows,:] = mask[0:rows-2,:]
    lefter[:,:cols-2] = mask[:,2:cols]
    righter[:,2:cols] = mask[:,:cols-2]
    ...
    # 这里的 conn[i] = mask * X 计算的是：
    # 如果 某个像素 和 相应方向偏移的像素 同时是前景（1），则该方向的连接性为 1，否则为 0。
    # 例如：如果 mask[x, y] = 1 且 down[x, y] = 1，说明这个像素与下方的像素是相连的。
    '''
    gt[i, j]这一点，对应的conn[0:8, i, j]的含义如下：
    例如，其中C1表明，conn[0, i, j]代表[i, j]与其下方偏移2个像素的位置的连通性，连通为1，否则为0
          C8
          C7
    C6 C5 ij C4 C3
          C2
...
def sal2conn_batch(masks):
    """
    将一批二值掩码转换成对应的连通性掩码
    masks shape: (n, h, w)
    返回的conn shape: (n, 8, h, w)
    """
    n, rows, cols = masks.shape  # n为批次数量，rows和cols为掩码的高度和宽度
    conn = torch.zeros([n, 8, rows, cols], device=masks.device)  # 初始化连接性掩码，形状 (n, 8, rows, cols)

    # 创建各方向偏移后的掩码（对于每个批次进行操作）
    up = torch.zeros([n, rows, cols], device=masks.device)
    down = torch.zeros([n, rows, cols], device=masks.device)
    left = torch.zeros([n, rows, cols], device=masks.device)
    right = torch.zeros([n, rows, cols], device=masks.device)
    upper = torch.zeros([n, rows, cols], device=masks.device)
    righter = torch.zeros([n, rows, cols], device=masks.device)
    lefter = torch.zeros([n, rows, cols], device=masks.device)
    downer = torch.zeros([n, rows, cols], device=masks.device)

    # 计算偏移后的掩码
    up[:, 1:, :] = masks[:, :-1, :]  # mask 向上移动 1 像素
    down[:, :-1, :] = masks[:, 1:, :]
    left[:, :, 1:] = masks[:, :, :-1]
    right[:, :, :-1] = masks[:, :, 1:]
    upper[:, 2:, :] = masks[:, :-2, :]
...
def bv_test(output_test): # 要求 output_test.shape = (1, 8, 512, 512)，即(b, 8, h, w)
    '''
