...
class SAM_Mask_Decoder(RSSamMaskDecoder):
    ...
    def forward(
        ...
        """
        batch_size, num_channels, height, width = image_embeddings.shape # 17,256,32,32
        point_batch_size = sparse_prompt_embeddings.shape[1] # 1
        # Concatenate output tokens
        output_tokens = torch.cat([self.mask_decoder.iou_token.weight, self.mask_decoder.mask_tokens.weight], dim=0) # (1,256)concat(4,256)=(5,256)
        output_tokens = output_tokens.repeat(batch_size, point_batch_size, 1, 1) # (17, 1, 1+4, 256)

        if sparse_prompt_embeddings.sum().item() != 0:
            tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=2) # (17, 1, 1+4+5, 256)
        else:
        ...
        point_embedding, image_embeddings, attentions = self.mask_decoder.transformer(
            point_embeddings=point_embeddings, # (17,1,10,256)
            image_embeddings=image_embeddings, # (17,256,32,32)
            image_positional_embeddings=image_positional_embeddings, # (17,256,32,32)
