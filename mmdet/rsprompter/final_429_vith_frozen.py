...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        if peft_config is None:  # 如果不使用 LORA微调，就冻结backbone
            ...
            self.frozen_modules += [self.backbone.vision_encoder_rgb]
            self.frozen_modules += [self.backbone.vision_encoder_depth]
...
class UCTNet_PretrainSamViT(BaseModule):
    def __init__(
        ...
        self.vision_encoder_depth = vision_encoder1

        self.vision_encoder_rgb.is_init = True
        self.vision_encoder_depth.is_init = True

        # self.FusionModules = nn.ModuleList([FusionModule(embed_dims=768) for _ in range(4)]) # vitb
        # self.FusionModules = nn.ModuleList([FusionModule(embed_dims=1024) for _ in range(4)]) # vitl
        self.FusionModules = nn.ModuleList([FusionModule(embed_dims=1280) for _ in range(4)]) # vith
        # self.out_indices=[2,5,8,11] # vitb
        # self.out_indices=[5,11,17,23] # vitl
        self.out_indices=[7,15,23,31] # vith
    ...
    def forward(self, batch_inputs):
        ...
        for i, (layer_rgb,layer_depth) in enumerate(zip(self.vision_encoder_rgb.layers, self.vision_encoder_depth.layers)):
            ...
            if i in self.out_indices: # (2,5,8,11)
                ...
                # fusion_i = i // 3 # (2,5,8,11)//3 = (0,1,2,3), vitb
                # fusion_i = i // 6 # (5,11,17,23)//3 = (0,1,2,3), vitl
                fusion_i = i // 8 # (7,15,23,31)//3 = (0,1,2,3), vith
