...
class SAM_Anchor_Prompt(MaskRCNN):
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # vit_features = rgb_outputs[0][0] # (b, 768, 32, 32)，
        # vit_features = rgb_outputs[0][3] # (b, 768, 32, 32)，
...
class WCAF_RGB(nn.Module):
    def __init__(
        ...
        self.depth_bias_proj = nn.Linear(embed_dims, embed_dims)
        # 利用简单卷积模块提取深度特征图中的局部结构信息
    ...
    def forward(self, SRC, TGT):
        ...
        # 计算depth_bias
        depth = SRC.permute(0, 2, 1).reshape(B, C, H, W) # (b,c,h,w)
        depth = self.depth_conv(depth) # 利用卷积提取空间结构信息，(b,c,h,w)
        depth = depth.flatten(2).permute(0, 2, 1) # (B, seq_len, 768)
        depth = self.depth_bias_proj(depth) # (B, seq_len, 768)
        depth_bias = depth @ depth.transpose(1, 2) * self.scale # (B, seq_len, 768) @ (B, 768, seq_len)
        # depth_bias = depth_bias.transpose(1, 2)  # [B, 1, seq_len]，转置一下，让注意力偏执矩阵在后续的计算中可以沿着query维度进行广播
...
class WCAF_Depth(nn.Module):
    def __init__(
        ...
        self.depth_bias_proj = nn.Linear(embed_dims, 1)
        # self.depth_bias_proj = nn.Sequential(
    ...
    def forward(self, SRC, TGT):
        ...
        depth_bias = self.depth_bias_proj(TGT)  # [B, seq_len, 1]
        depth_bias = depth_bias.transpose(1, 2)  # [B, 1, seq_len]，转置一下，让注意力偏执矩阵在后续的计算中可以沿着query维度进行广播
...
class FusionModule(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.embed_dims = embed_dims
        self.RGB_WCAF = WCAF_RGB(embed_dims)
        # self.Depth_WCAF = WCAF_Depth(embed_dims)
