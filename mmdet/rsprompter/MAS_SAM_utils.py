...
class desam(nn.Module):
    ...
    def forward(self, mask_in, mask_embed):
        '''
        # mask_in就是src，形状(b,256,32,32)，是image_embedding与提示嵌入、iou_token、mask_tokens进行了TwoWay交叉注意力后得到的结果
        mask_embed是[res[3],res[2],res[1],res[0]]，代表image_encoder第12、9、6、3层全局注意力块的输出, 形状均为(b,32,32,768)
        '''
        # pre_deal
        de12_ = self.de12_(mask_embed[0].permute(0, 3, 1, 2))  # b,256,32,32
        ...
        de12 = self.de12(mask_embed[0].permute(0, 3, 1, 2))  # b,256,32,32
        ...
        hyper = torch.cat([de12_, de9_, de6_, de3_], dim=1)  # b,256*4,32,32
        hyper = self.hyper(hyper)  # (b,256,32,32)，融合res[3~0]，得到hyper
        hyper1 = self.hyper_up1(hyper)  # (b,128,64,64)，将融合所得 hyper经过上采样，得到通道数更少、分辨率更大的 hyper1~3
        hyper2 = self.hyper_up2(hyper1)  # (b,64,128,128)
        hyper3 = self.hyper_up3(hyper2)  # (b,32,256,256)

        mask1 = torch.cat([mask_in, de12, hyper], dim=1)  # (b,768,32,32)
        mask1_ = self.fu1(mask1)  # b,256,32,32
        mask1 = self.up1(mask1_)  # b,256,64,64
        ...
        mask_pre4 = self.pre4(mask4)  # b,2,256,256
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        masks, iou_pred = self.predict_masks(
            image_embeddings=image_embeddings,  # tuple_5
            image_pe=image_pe,  # (1,256,32,32)
            sparse_prompt_embeddings=sparse_prompt_embeddings,  # 空
            dense_prompt_embeddings=dense_prompt_embeddings,  # (1,256,32,32)
        )
    ...
    def predict_masks(
        ...
        de_in = image_embeddings[1:]  # res[3~0]，代表image_encoder第12、9、6、3层全局注意力块的输出, (b,32,32,768)
        image_embeddings = image_embeddings[0]  # x, 代表image_encoder的最终输出，(b,256,32,32)
        # Concatenate output tokens
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)  # (3,256)
        output_tokens = output_tokens.unsqueeze(0).expand(sparse_prompt_embeddings.size(0), -1, -1)  # (1,3,256)
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)  # (1,3,256)

        # Expand per-image data in batch direction to be per-mask
        # print(src.size())
        if dense_prompt_embeddings.size(0) == 1:
            src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)  # (b,256,32,32)
        else:
            src = image_embeddings
        # print(src.size())
        # print(dense_prompt_embeddings.size())
        src = src + dense_prompt_embeddings  # (2,256,32,32)
        pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)  # (1,256,32,32), 位置编码
        b, c, h, w = src.shape
        ...
        src = src.transpose(1, 2).view(b, c, h, w)  # torch.Size([2, 256, 32, 32])
        res = self.desam(src,
                         de_in)  # src是image_embedding与提示嵌入、iou_token、mask_tokens进行了TwoWay交叉注意力后得到的结果，de_in是res[3~0]，代表image_encoder第12、9、6、3层全局注意力块的输出, (b,32,32,768)
        ...
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h,
                                                                       w)  # [1, 4, 256, 256], 256 = 4 * 64, the size of image embeddings
