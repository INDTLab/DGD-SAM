...
class SAM_Anchor_Prompt(MaskRCNN):
    def __init__(
        ...
        self.backbone.vision_encoder.base_model.model.out_indices = [2, 5, 8, 11]
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        rgb1 = rgb_outputs[0][0] # (b,768,32,32)
        b, c, h, w = rgb1.shape
        center_vector = rgb1[:, :, 5, 5]  # 取中心点特征
        feature_map_flat = rgb1.view(b, c, -1)  # 变为 (b, 768, 32*32)
        similarity_map = torch.einsum('bc, bcn -> bn', center_vector, feature_map_flat)  # 点积相似度 (b, 128*128)
        similarity_map = similarity_map.view(b, h, w)  # 变回 (b, 32, 32)
        similarity_map = (similarity_map - similarity_map.min()) / (similarity_map.max() - similarity_map.min())

        rgb2 = rgb_outputs[0][3] # (b,768,32,32)
        b, c, h, w = rgb2.shape
        center_vector = rgb2[:, :, 5, 5]  # 取中心点特征
        feature_map_flat = rgb2.view(b, c, -1)  # 变为 (b, 32, 128*128)
        ...
        plt.scatter(5, 5, color='red', s=100, marker='o')
        plt.subplot(1, 2, 2)
        ...
        vit_features = rgb_outputs[0][1] # (b, 768, 32, 32)
...
@MODELS.register_module()
class LoRA_Adapter_PretrainSamVisionEncoder(BaseModule):
    def __init__(
            self,
            hf_pretrain_name,
            img_size=1024,
    ...
    def forward(self, *args, **kwargs):
        return self.vision_encoder(*args, **kwargs)
@MODELS.register_module()
class LoRA_Adapter_ViTSAM(ViTSAM):
    ...
    def __init__(self,
        ...
        dpr = np.linspace(0, 0., self.num_layers)
        self.layers = ModuleList()

        layer_cfgs = [kwargs.get('layer_cfgs', {})] * self.num_layers
        for i in range(self.num_layers):
            _layer_cfg = dict(
                ...
                use_rel_pos=kwargs.get('use_rel_pos',True),
                norm_cfg=dict(type='LN', eps=1e-6)
            )
            _layer_cfg.update(layer_cfgs[i])
            self.layers.append(My_Layer(**_layer_cfg))
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, self.adapter[i])

            if i in self.out_indices:
                # (B, H, W, C) -> (B, C, H, W)
                x_reshape = x.permute(0, 3, 1, 2)
                inter_embeddings.append(x_reshape)
...
class My_Layer(TransformerEncoderLayer):
    ...
    def __init__(self,
        ...
        super().__init__(*args, **kwargs)
        # self.mlp = MLPBlock(embedding_dim=768, mlp_dim=int(768 * 4.0), act=nn.GELU)
        self.ffn = My_FFN(
            embed_dims=kwargs.get('embed_dims'),
            feedforward_channels=kwargs.get('feedforward_channels'),
            num_fcs=2,
            ffn_drop=kwargs.get('drop_rate'),
            dropout_layer=dict(type='DropPath', drop_prob=kwargs.get('drop_path_rate')),
    ...
    def forward(self, x, adapter=None):
        ...
        # x = self.ffn(self.ln2(x), identity=x) # self.ffn()操作，把残差部分也做上了，不符合我的需要
        x = x + adapter(self.ffn(self.ln2(x))) # 重写了ffn()，去掉了ffn内部的残差连接，改由此处实现残差连接
        return x
