...
class USISAnchor(MaskRCNN):
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        if isinstance(vision_outputs, SamVisionEncoderOutput):
            image_embeddings = vision_outputs[0] # 图像编码器的最终输出
            vision_hidden_states = vision_outputs[1] # 图像编码器的中间层输出
...
class USISSimpleFPNHead(BaseModule):
    ...
    def forward(self, input: Tensor) -> tuple: # input.shape = (2, 256, 32, 32), BCHW
        """Forward function.
        ...
        """
        # build FPN
        inputs = [] # 初始 input.shape = (2, 256, 32, 32), BCHW
        inputs.append(self.fpn1(input)) # (2, 64, 128, 128), BCHW
        inputs.append(self.fpn2(input)) # (2, 128, 64, 64), BCHW
        inputs.append(self.fpn3(input)) # (2, 256, 32, 32), BCHW
        inputs.append(self.fpn4(input)) # (2, 256, 16, 16), BCHW
...
class USISFeatureAggregator(BaseModule):
    ...
    def forward(self, inputs): # input是长度为 33的tuple
        assert len(inputs) == len(self.in_channels)
        inputs = [einops.rearrange(x, 'b h w c -> b c h w') for x in inputs]

        features = []
        for idx, i_layer in enumerate(self.select_layers): # self.select_layers = range(8, 33, 2)
            features.append(
                self.downconvs[idx](inputs[i_layer] * self.ca_adapter(inputs[i_layer])) # self.downconvs是 13个 MultiScaleConv组成的 ModuleList
            )

        x = None
        for hidden_state, hidden_conv in zip(features, self.hidden_convs): # feature是 list_13, self.hidden_convs是 ModuleList_13
