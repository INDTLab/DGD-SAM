...
def log_current_file():
    """将当前模型文件内容记录到 MMDetection 日志文件中"""
    file_path = os.path.abspath(__file__)  # 获取当前文件的路径
    logger = MMLogger.get_current_instance()  # 获取当前训练的 Logger
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            file_content = f.read()
        logger.info(f"\n===== Logging File: {file_path} =====\n{file_content}\n")  # 记录日志
    else:
...
class UCTNet_PretrainSamViT(BaseModule):
    def __init__(
        ...
        self.CoarseAttnModules = nn.ModuleList([
            CoarseAttnModule(in_channels=768) for _ in range(4)
        ])
        # self.CoarseAttnModules = CoarseAttnModule(in_channels=768)
        # 可选：门控融合模块（1x1卷积），更精细的融合策略
        self.gate_conv = nn.Conv2d(768 * 2, 768, kernel_size=1)
    ...
    def forward(self, batch_inputs):
        ...
        for i, (layer_rgb,layer_depth) in enumerate(zip(self.vision_encoder_rgb.layers, self.vision_encoder_depth.layers)):
            ...
            if i in self.out_indices: # (2,5,8,11)
                ...
                # 在细粒度融合后引入粗粒度注意力模块
                coarse_attn = self.CoarseAttnModules[fusion_i](new_rgb_feature) # (B,768,32,32)
                # coarse_attn = self.CoarseAttnModules[fusion_i](new_depth_feature) # (B,768,32,32)
                # 方法1：加权乘法（确保初始信息不丢失，可采用 1+的方式）
                # fused_feature = new_rgb_feature * (1 + coarse_attn) # (B,768,32,32)

                # 方法2：门控融合（需提前定义 gate_conv）
                fusion_gate = torch.sigmoid(self.gate_conv(torch.cat([new_rgb_feature, coarse_attn], dim=1)))
                fused_feature = fusion_gate * new_rgb_feature + (1 - fusion_gate) * coarse_attn
                # fused_feature = fusion_gate * new_rgb_feature + (1 - fusion_gate) * (new_rgb_feature * coarse_attn)
                # fused_feature = fused_feature + new_rgb_feature
                # 方法3：self.alpha
                # fused_feature = self.alpha * new_rgb_feature + (1 - self.alpha) * (new_rgb_feature * coarse_attn)

                # 更新 RGB 分支，供后续处理
                rgb = fused_feature.permute(0,2,3,1) # (B,32,32,768)
...
class CoarseAttnModule(nn.Module):
    def __init__(self, in_channels=768, reduction=4):
        super(CoarseAttnModule, self).__init__()
        # 1. 下采样（将32x32下采样到8x8，采样因子4）
        ...
        # 2. 结构信息提取模块（采用空洞卷积扩大感受野）
        self.structure_conv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=2, dilation=2),
            nn.ReLU(inplace=True)
        )
        # 3. 自注意力或 MLP 提取全局信息，生成粗粒度注意力图
        # 这里简单使用 1x1 卷积获得注意力图，通道数变为 in_channels // reduction
        self.attn_conv = nn.Conv2d(in_channels, in_channels // reduction, kernel_size=1)
    ...
    def forward(self, x):
        # x: (B, C, H, W) 假设 H=W=32
        # 1. 下采样：得到低分辨率特征
        low_res_feat = self.downsample(x)  # (B, C, 8, 8)
        # 2. 提取结构信息
        structure_feat = self.structure_conv(low_res_feat)  # (B, C, 8, 8)
        # 3. 得到注意力图
        attn = self.attn_conv(structure_feat)  # (B, C//reduction, 8, 8)
        attn = self.attn_mlp(attn)  # (B, C, 8, 8)，值域在(0,1)
        # 4. 上采样到原始分辨率（32x32）
        attn_up = F.interpolate(attn, size=x.shape[2:], mode='bilinear', align_corners=False)
        # 5. 返回粗粒度注意力图
        return attn_up
...
class MultiScaleFusion(nn.Module):
    def __init__(self, embed_dims):
        super().__init__()
        self.pixel_fusion = WCAF(embed_dims)      # 原来的像素级注意力模块
        self.edge_attn = EdgeAttention(in_channels=embed_dims, out_channels=embed_dims)
        # 融合两种尺度的权重，可以采用逐元素相乘或加权求和，下面用加权和作为示例
        self.fusion_weight = nn.Parameter(torch.tensor(0.5))  # 可学习的融合比例

    def forward(self, rgb_feature, depth_feature):
        # 细粒度分支：原始 WCAF
        fine_rgb_feature = self.pixel_fusion(SRC=depth_feature, TGT=rgb_feature)
        fine_depth_feature = self.pixel_fusion(SRC=rgb_feature, TGT=depth_feature)

        # 粗粒度分支：利用深度特征计算边缘注意力，生成粗权重
        # 注意：这里可以直接用深度特征，也可以试验将深度图融合到中间特征再进行边缘提取
        coarse_attn_rgb = self.edge_attn(depth_feature)  # 对应于 RGB 分支的深度指导
        coarse_attn_depth = self.edge_attn(rgb_feature)   # 对应于 Depth 分支的 RGB 辅助（根据具体情况调整）

        # 生成最终融合权重（例如使用加权求和）
        new_rgb_feature = self.fusion_weight * fine_rgb_feature + (1 - self.fusion_weight) * (rgb_feature * coarse_attn_rgb)
        new_depth_feature = self.fusion_weight * fine_depth_feature + (1 - self.fusion_weight) * (depth_feature * coarse_attn_depth)
...
class GFA(nn.Module):
    def __init__(self):
        ...
        self.pos_encode = nn.Conv2d(self.dim * 2, self.dim * 2, (3,3), (1,1), (1,1),
                                    groups=self.dim * 2)
        self.ln_norm1 = nn.LayerNorm(self.dim, eps=1e-6)
        # self.attn = CrossAttention(cfg.trans.attn, stage)

        self.proj = nn.Conv2d(self.dim * 2, self.dim, (1,1), bias=False)
        self.residual = nn.Sequential(
            nn.Conv2d(self.dim * 2, self.dim, (1,1), bias=False),
            nn.Conv2d(self.dim, self.dim, (3,3), (1,1), (1,1), groups=self.dim,
                      bias=False),
            nn.ReLU(inplace=True),
