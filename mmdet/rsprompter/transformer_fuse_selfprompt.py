...
class SAM_Mask_Decoder(RSSamMaskDecoder):
    ...
    def forward(
        ...
        # tmp = upscaled_embedding.squeeze(1).permute(0,2,1) # (batch,128*128,32)
        # pca = PCA(n_components=2)
        # features_2d = pca.fit_transform(tmp[0].detach().cpu().numpy())  # shape: (H*W, 2)
        # # 只用第一个主成分
        # component1 = features_2d[:, 0].reshape(height, width)
        # plt.imshow(component1, cmap='viridis')
        # plt.colorbar()
        # plt.title("PCA Component 1 Heatmap")
        # plt.show()
        # tmp = upscaled_embedding.squeeze(1).reshape(batch_size, num_channels, height, width) # (B,32,128,128)
        # feat_avg = tmp[0].mean(dim=0).detach().cpu().numpy()
        # feat_norm = (feat_avg - feat_avg.min()) / (feat_avg.max() - feat_avg.min() + 1e-5)
        # feat_colormap = cv2.applyColorMap(np.uint8(feat_norm * 255), cv2.COLORMAP_JET)
        # # 如果需要显示或保存：
        # plt.imshow(cv2.cvtColor(feat_colormap, cv2.COLOR_BGR2RGB))
        # plt.axis('off')
        # plt.show()
        #
        # show(upscaled_embedding, 0)
        # plt.subplot(1, 2, 1)
        # plt.title('origin')
...
import cv2
def show_upscaled_embedding(upscaled_embedding:Tensor, idx=0):
    batch_size= upscaled_embedding.shape[0]
    tmp = upscaled_embedding.squeeze(1).reshape(batch_size, 32, 128, 128)  # (B,32,128,128)
    tmp = F.interpolate(tmp, (512, 512), mode='bilinear', align_corners=True)
    feat_avg = tmp[idx].mean(dim=0).detach().cpu().numpy()
    feat_norm = (feat_avg - feat_avg.min()) / (feat_avg.max() - feat_avg.min() + 1e-5)
    feat_colormap = cv2.applyColorMap(np.uint8(feat_norm * 255), cv2.COLORMAP_JET)
...
def show_masks(masks,idx=0,cmap='viridis'):
    tmp = masks.squeeze(1) # (b,4,128,128)
    tmp = F.interpolate(tmp, (512,512), mode='bilinear', align_corners=True)
    plt.imshow(tmp.detach().cpu().numpy()[idx,0,:,:], cmap=cmap)
    plt.show()