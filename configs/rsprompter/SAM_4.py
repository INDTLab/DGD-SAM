...
data_preprocessor = dict(
    ...
    pad_size_divisor=32,
    # 新增深度图参数
    depth_mean=0.450*255, # 根据实际深度图统计调整
    depth_std=0.268*255, # 示例值，需替换为实际值
...
#
#     # 同步空间变换模块
#     dict(type='DualRandomFlip', prob=0.5),  # 替换原有RandomFlip
#
#     # 同步缩放
#     dict(
#         type='DualRandomResize',  # 替换原有RandomResize
#         scale=crop_size,
#         ratio_range=(0.1, 2.0),
#         resize_type='Resize',
#         keep_ratio=True,
#         depth_interp='nearest'),  # 深度图使用最近邻插值
#
#     # 同步裁剪
#     dict(
#         type='DualRandomCrop',  # 替换原有RandomCrop
#         crop_size=crop_size,
#         crop_type='absolute',
#         recompute_bbox=True,
#         allow_negative_crop=True,
#         depth_pad_val=0),  # 深度图填充值
#
#     # 过滤标注（无需修改）
#     dict(type='FilterAnnotations', min_gt_bbox_wh=(1e-5, 1e-5), by_mask=True),
#
#     # 修改后的数据打包
#     dict(type='DualPackDetInputs',
#          meta_keys=('img_id', 'img_path', 'depth_path', 'ori_shape', 'img_shape',
...
test_pipeline = [
    ...
    # 中经过了pad的部分变成 0了
    dict(type='Pad', size=crop_size, pad_val=dict(img=(0.406 * 255, 0.456 * 255, 0.485 * 255, 0.450*255), masks=0)),
...
#
#     # 同步缩放（需实现双模态版本）
#     dict(type='DualResize',  # 替换原有Resize
#          scale=crop_size,
#          keep_ratio=True,
#          depth_interp='nearest'),
#
#     # 同步填充（需实现双模态版本）
#     dict(type='DualPad',  # 替换原有Pad
#          size=crop_size,
#          pad_val=dict(
#              img=(0.406 * 255, 0.456 * 255, 0.485 * 255),  # RGB填充值
#              depth=0  # 深度图填充值（根据实际需求设置）
#          )),
#
#     # 加载标注（如果测试时需要评估）
#     dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
...
#     # 修改后的数据打包
#     dict(type='DualPackDetInputs',
#          meta_keys=('img_id', 'img_path', 'depth_path',  # 新增depth_path
