...
model = dict(
    ...
    panoptic_head=dict(
        type='MyMask2FormerHead',
        # decoder_plus=True,
        decoder_plus=False,
        mask_decoder=dict(
            # type='RSSamMaskDecoder',
            type='MySamMaskDecoder_Fastinst',
            hf_pretrain_name=hf_sam_pretrain_name,
            init_cfg=dict(type='Pretrained', checkpoint=hf_sam_pretrain_ckpt_path)),
...
# 修改batch，记得也要修改 image_embeddings = image_embeddings.repeat_interleave(torch.tensor([70, 70]), dim=0) # (140, 256, 16, 16)
...
auto_scale_lr = dict(enable=False, base_batch_size=batch_size_per_gpu)
