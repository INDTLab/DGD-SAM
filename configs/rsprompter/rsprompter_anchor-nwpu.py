...
work_dir = './work_dirs/rsprompter/rsprompter_anchor-nwpu'
...
vis_backends = [dict(type='LocalVisBackend'),
                dict(type='WandbVisBackend', init_kwargs=dict(project='rsprompter-nwpu', group='rsprompter-anchor', name='rsprompter-anchor-nwpu'))
                ]
visualizer = dict(
...
model = dict(
    ...
    backbone=dict(
        # 不加peft_config=None, 就会调整整个image encoder的参数
        peft_config=None,
        hf_pretrain_name=hf_sam_pretrain_name,
        init_cfg=dict(type='Pretrained', checkpoint=hf_sam_pretrain_ckpt_path)
    ),
