...
#                 # pipeline=pre_copy_paste_pipeline
#                 pipeline=[]
#             )
#         ),
...
train_dataloader = dict(
    ...
    dataset=dict(
        type='ClassBalancedDataset',
        # oversample_thr=0.001, # 根据类别不平衡程度设置，建议 0.001～0.01
        oversample_thr=0.015, # 根据类别不平衡程度设置，建议 0.001～0.01
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
            ann_file="/data2/yihan/MyDataset/KITTI_semantics/training/kitti_instances_coco_1_8.json",
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        # ann_file="/data2/yihan/MyDataset/KITTI_semantics/training/kitti_instances_coco.json",
        # ann_file="/data2/yihan/MyDataset/KITTI_semantics/training/kitti_instances_coco_1_19.json",
        ann_file="/data2/yihan/MyDataset/KITTI_semantics/training/kitti_instances_coco_1_8.json", # KITTI的测试集没有标注文件，所以就不加 ann_file了
        # ann_file="/data3/yihan/MyDataset/KITTI_semantics/training/kitti_instances_coco_1_8.json", # KITTI的测试集没有标注文件，所以就不加 ann_file了
        # ann_file="/data3/yihan/MyDataset/KITTI_semantics/split/annotations_val_split.json", # KITTI的测试集没有标注文件，所以就不加 ann_file了
        data_prefix=dict(img="training/image_2"),
        # data_prefix=dict(img="split/val_images"),
